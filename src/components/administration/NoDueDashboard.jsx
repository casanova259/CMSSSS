...
const CLEARANCE_LABELS = { library: 'Library', hostel: 'Hostel', accounts: 'Accounts', department: 'Dept', examCell: 'Exam Cell' };
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                {['Department', 'Total Students', 'Cleared', 'Pending', 'Completion %'].map((h, i) => (
                  <th key={i} className="px-6 py-3 text-left text-sm font-semibold text-gray-700">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                {['Student Name', 'Roll No', 'Dept', ...Object.values(CLEARANCE_LABELS), 'Status', 'Actions'].map((h, i) => (
                  <th key={i} className="px-4 py-3 text-left text-sm font-semibold text-gray-700">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {noDueApplications.map(app => (
                <tr key={app.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-4 text-sm text-gray-900">{app.fullName}</td>
                  <td className="px-4 py-4 text-sm text-gray-900">{app.rollNo}</td>
                  <td className="px-4 py-4 text-sm text-gray-900">{app.department}</td>
                  ...
                  <td className="px-4 py-4">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${app.status === 'complete' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                      {app.status === 'complete' ? 'Complete' : 'Pending'}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <button onClick={() => setSelectedApplication(app)} className="text-blue-500 hover:text-blue-600">
                      <Eye size={20} />
                    </button>
                  </td>
                </tr>
          ...
          {noDueApplications.length === 0 && (
            <div className="text-center py-12 text-gray-500"><p>No no-due applications found</p></div>
          )}
        </div>
      </div>
